...
class ResourceCandidate:
    ...
    def as_location(self) -> Location:
        return Location(self.id, self.name, self.lat, self.lng)
...
class DispatchScenario:
    ...
    landing_candidates: List[ResourceCandidate]
    truck_candidates: List[ResourceCandidate]
    uav_candidates: List[ResourceCandidate]
    robot_candidates: List[ResourceCandidate]
    source_prompt: str = ""
...
class GeneticDispatchOptimizer:
    ...
    与上面的通用TSP版GA不同，这里优化的是离散组合：
    - 起点仓库
    - 中转起降点
    - Robot/Truck/UAV 资源选择

    评价函数采用项目现有模块可复用的近似代价：
    - Robot 从所属仓库移动到出货仓库的代价
    - Truck 从所属仓库到出货仓库，再到中转起降点的代价
    - UAV 从所属仓库到中转点，再到最终目的地的代价

    这样可以把 GA 真正接入“高层调度”这一层，而不只是做一个与项目无关的路径排序示例。
    """
    ...
    def _create_individual(self, scenario: DispatchScenario) -> List[int]:
        return [
            self.random.randrange(len(scenario.warehouse_candidates)),
            self.random.randrange(len(scenario.landing_candidates)),
            self.random.randrange(len(scenario.truck_candidates)),
            self.random.randrange(len(scenario.uav_candidates)),
            self.random.randrange(len(scenario.robot_candidates)),
        ]

    def _distance(self, src: ResourceCandidate, dst: ResourceCandidate) -> float:
        return src.as_location().distance_to(dst.as_location())

    def _estimate_cost(self, individual: List[int], scenario: DispatchScenario) -> Dict[str, Any]:
        warehouse = scenario.warehouse_candidates[individual[0]]
        landing = scenario.landing_candidates[individual[1]]
        truck = scenario.truck_candidates[individual[2]]
        uav = scenario.uav_candidates[individual[3]]
        robot = scenario.robot_candidates[individual[4]]
        destination = ResourceCandidate(
            id=scenario.destination.id,
            name=scenario.destination.name,
            lat=scenario.destination.lat,
            lng=scenario.destination.lng,
            resource_type="destination",
        )

        # 近似速度（km/h），用于把距离转成物理业务时间
        robot_speed = float(robot.metadata.get("speed_kmh", 3.6))   # 1m/s ≈ 3.6km/h
        truck_speed = float(truck.metadata.get("speed_kmh", 45.0))
        uav_speed = float(uav.metadata.get("speed_kmh", 36.0))

        robot_to_warehouse_km = self._distance(robot, warehouse)
        truck_to_warehouse_km = self._distance(truck, warehouse)
        truck_route_km = self._distance(warehouse, landing)
        uav_to_landing_km = self._distance(uav, landing)
        uav_route_km = self._distance(landing, destination)

        robot_time = robot_to_warehouse_km / max(robot_speed, 0.1) * 3600 + 120.0
        truck_time = (truck_to_warehouse_km + truck_route_km) / max(truck_speed, 0.1) * 3600
        uav_time = (uav_to_landing_km + uav_route_km) / max(uav_speed, 0.1) * 3600

        penalty = 0.0
        # 鼓励选择离终点更近的中转起降点
        penalty += uav_route_km * 30.0
        # 鼓励 Truck 和 UAV 在空间上更平滑接驳
        penalty += abs(truck_route_km - uav_route_km) * 5.0

        total_cost = robot_time + truck_time + uav_time + penalty
        return {
            "warehouse": warehouse,
            "landing": landing,
            "truck": truck,
            ...
            "truck_route_km": truck_route_km,
            "uav_route_km": uav_route_km,
        }

    def _fitness(self, individual: List[int], scenario: DispatchScenario) -> float:
        cost = self._estimate_cost(individual, scenario)["total_cost"]
    ...
    def _crossover(self, parent1: List[int], parent2: List[int], scenario: DispatchScenario) -> Tuple[List[int], List[int]]:
        ...
        bounds = [
            len(scenario.warehouse_candidates),
            len(scenario.landing_candidates),
            len(scenario.truck_candidates),
            len(scenario.uav_candidates),
            len(scenario.robot_candidates),
        ]
        child1 = [gene % bound for gene, bound in zip(child1, bounds)]
        child2 = [gene % bound for gene, bound in zip(child2, bounds)]
        return child1, child2

    def _mutate(self, individual: List[int], scenario: DispatchScenario) -> List[int]:
        bounds = [
            len(scenario.warehouse_candidates),
            len(scenario.landing_candidates),
            len(scenario.truck_candidates),
        ...
        for idx, bound in enumerate(bounds):
            if self.random.random() < self.mutation_rate:
                mutated[idx] = self.random.randrange(bound)
        return mutated

    def optimize_dispatch(self, scenario: DispatchScenario) -> OptimizationResult:
        if not all([
            ...
            raise ValueError("调度候选集不完整，无法运行GA对比实验")

        population = [self._create_individual(scenario) for _ in range(self.population_size)]
        ...
        for _ in range(self.generations):
            fitnesses = [self._fitness(ind, scenario) for ind in population]
            evaluations = [self._estimate_cost(ind, scenario) for ind in population]

            generation_best_idx = min(range(len(population)), key=lambda idx: evaluations[idx]["total_cost"])
            generation_best = evaluations[generation_best_idx]

            if initial_best_cost is None:
                initial_best_cost = generation_best["total_cost"]

            if generation_best["total_cost"] < best_cost:
            ...
            while len(new_population) < self.population_size:
                ...
                child1, child2 = self._crossover(parent1, parent2, scenario)
                child1 = self._mutate(child1, scenario)
                child2 = self._mutate(child2, scenario)
                new_population.extend([child1, child2])

            population = new_population[:self.population_size]

        if best_individual is None or best_eval is None:
            raise RuntimeError("遗传算法未能生成有效解")

        improvement_ratio = 0.0
        if initial_best_cost and initial_best_cost > 0:
            improvement_ratio = (initial_best_cost - best_cost) / initial_best_cost

        route_tokens = [
            f"warehouse:{best_eval['warehouse'].id}",
            f"landing:{best_eval['landing'].id}",
            f"truck:{best_eval['truck'].id}",
            f"uav:{best_eval['uav'].id}",
            f"robot:{best_eval['robot'].id}",
        ]

        return OptimizationResult(
            algorithm_name="GA_DISPATCH",
            best_route=route_tokens,
            best_distance=best_eval["truck_route_km"] + best_eval["uav_route_km"],
            best_time=best_eval["robot_time_s"] + best_eval["truck_time_s"] + best_eval["uav_time_s"],
            iterations=self.generations,
            improvement_ratio=improvement_ratio,
            convergence_history=convergence_history,
            metadata={
                "warehouse": {
                    "id": best_eval["warehouse"].id,
                    "name": best_eval["warehouse"].name,
                    "lat": best_eval["warehouse"].lat,
                    "lng": best_eval["warehouse"].lng,
                },
                "landing": {
                    "id": best_eval["landing"].id,
                    "name": best_eval["landing"].name,
                    "lat": best_eval["landing"].lat,
                    "lng": best_eval["landing"].lng,
                },
                "truck": {
                    "id": best_eval["truck"].id,
                    "name": best_eval["truck"].name,
                },
                "uav": {
                    "id": best_eval["uav"].id,
                    "name": best_eval["uav"].name,
                },
                "robot": {
                    "id": best_eval["robot"].id,
                    "name": best_eval["robot"].name,
                },
                "cost_breakdown": {
                    "robot_time_s": best_eval["robot_time_s"],
                    "truck_time_s": best_eval["truck_time_s"],
                    "uav_time_s": best_eval["uav_time_s"],
...
# ===================== ???????? =====================
class AntColonyDispatchOptimizer:
    """
    ?????????????????
    ...
    def _estimate_cost(self, solution: List[int], scenario: DispatchScenario) -> Dict[str, Any]:
        warehouse = scenario.warehouse_candidates[solution[0]]
        landing = scenario.landing_candidates[solution[1]]
        truck = scenario.truck_candidates[solution[2]]
        uav = scenario.uav_candidates[solution[3]]
        robot = scenario.robot_candidates[solution[4]]
        destination = ResourceCandidate(
            id=scenario.destination.id,
            name=scenario.destination.name,
            lat=scenario.destination.lat,
        ...
        robot_speed = float(robot.metadata.get("speed_kmh", 3.6))
        truck_speed = float(truck.metadata.get("speed_kmh", 45.0))
        uav_speed = float(uav.metadata.get("speed_kmh", 36.0))
        ...
        penalty = uav_route_km * 30.0 + abs(truck_route_km - uav_route_km) * 5.0
        total_cost = robot_time + truck_time + uav_time + penalty
    ...
    def _heuristic(self, category_idx: int, candidate_idx: int, scenario: DispatchScenario) -> float:
        if category_idx == 0:
            candidate = scenario.warehouse_candidates[candidate_idx]
            return 1.0 / max(candidate.as_location().distance_to(scenario.destination), 1e-3)
        if category_idx == 1:
            candidate = scenario.landing_candidates[candidate_idx]
            return 1.0 / max(candidate.as_location().distance_to(scenario.destination), 1e-3)
        return 1.0

    def _candidate_count(self, category_idx: int, scenario: DispatchScenario) -> int:
        counts = [
            len(scenario.warehouse_candidates),
            len(scenario.landing_candidates),
            len(scenario.truck_candidates),
        ...
        return counts[category_idx]

    def _choose_gene(self, category_idx: int, scenario: DispatchScenario) -> int:
        count = self._candidate_count(category_idx, scenario)
        scores: List[float] = []
        total_score = 0.0
        for candidate_idx in range(count):
            pheromone = self.pheromones[category_idx][candidate_idx] ** self.alpha
            heuristic = self._heuristic(category_idx, candidate_idx, scenario) ** self.beta
            score = pheromone * heuristic
            scores.append(score)
            total_score += score

        if total_score <= 0:
            return self.random.randrange(count)

        target = self.random.uniform(0, total_score)
        cumulative = 0.0
    ...
    def optimize_dispatch(self, scenario: DispatchScenario) -> OptimizationResult:
        if not all([
            ...
            raise ValueError("????????????? ACO ????")

        self.pheromones = [
            [1.0 for _ in scenario.warehouse_candidates],
            [1.0 for _ in scenario.landing_candidates],
            [1.0 for _ in scenario.truck_candidates],
            [1.0 for _ in scenario.uav_candidates],
            [1.0 for _ in scenario.robot_candidates],
        ]

        best_solution: Optional[List[int]] = None
        best_eval: Optional[Dict[str, Any]] = None
        best_cost = float("inf")
        initial_best_cost: Optional[float] = None
        convergence_history: List[float] = []

        for _ in range(self.iterations):
            iteration_solutions: List[Tuple[List[int], Dict[str, Any]]] = []

            for _ant in range(self.num_ants):
                solution = [self._choose_gene(category_idx, scenario) for category_idx in range(5)]
                evaluation = self._estimate_cost(solution, scenario)
                iteration_solutions.append((solution, evaluation))

                if initial_best_cost is None or evaluation["total_cost"] < initial_best_cost:
                    initial_best_cost = evaluation["total_cost"]
            ...
            for category in self.pheromones:
                for idx in range(len(category)):
                    category[idx] *= (1.0 - self.rho)
                    if category[idx] < 1e-6:
                        category[idx] = 1e-6

            for solution, evaluation in iteration_solutions:
                deposit = self.q / max(evaluation["total_cost"], 1e-6)
                for category_idx, candidate_idx in enumerate(solution):
                    self.pheromones[category_idx][candidate_idx] += deposit

            if best_solution is not None and best_eval is not None:
                elite_deposit = self.q / max(best_eval["total_cost"], 1e-6)
                for category_idx, candidate_idx in enumerate(best_solution):
                    self.pheromones[category_idx][candidate_idx] += elite_deposit

        if best_solution is None or best_eval is None:
            raise RuntimeError("ACO ???????????")

        improvement_ratio = 0.0
        if initial_best_cost and initial_best_cost > 0:
            improvement_ratio = max(0.0, (initial_best_cost - best_cost) / initial_best_cost)

        return OptimizationResult(
            algorithm_name="ACO_DISPATCH",
            best_route=[
                f"warehouse:{best_eval['warehouse'].id}",
                f"landing:{best_eval['landing'].id}",
                f"truck:{best_eval['truck'].id}",
            ...
            best_distance=best_eval["truck_route_km"] + best_eval["uav_route_km"],
            best_time=best_eval["total_cost"],
