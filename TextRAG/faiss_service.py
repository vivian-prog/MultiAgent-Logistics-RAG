...
INPUT_DIR = os.environ.get(
    "TEXTRAG_INPUT_DIR",
    os.path.join(os.path.dirname(__file__), "..", "GraphRag", "inputs", "artifacts")
)
TEXT_UNIT_TABLE = "create_final_text_units"
FAISS_INDEX_DIR = os.path.join(os.path.dirname(__file__), "faiss_index")
FAISS_INDEX_FILE = os.path.join(FAISS_INDEX_DIR, "text_vectors.index")
METADATA_FILE = os.path.join(FAISS_INDEX_DIR, "text_metadata.json")
...
PORT = int(os.environ.get("TEXTRAG_PORT", "8016"))
...
# ===================== 数据加载函数 =====================
def load_text_units_from_parquet() -> List[Dict[str, Any]]:
    parquet_path = os.path.join(INPUT_DIR, f"{TEXT_UNIT_TABLE}.parquet")
    if not os.path.exists(parquet_path):
        raise FileNotFoundError(f"找不到text_units文件: {parquet_path}")

    df = pd.read_parquet(parquet_path)
    logger.info(f"从 {parquet_path} 加载了 {len(df)} 条text_units")
    ...
    for idx, row in df.iterrows():
        ...
        text_units.append({
            ...
            "text": row.get("text", ""),
            "document_ids": list(doc_ids) if doc_ids is not None else [],
            "entity_ids": list(ent_ids) if ent_ids is not None else [],
            "relationship_ids": list(rel_ids) if rel_ids is not None else []
        })
    return text_units
    ...
    for score, idx in zip(scores[0], indices[0]):
        if 0 <= idx < len(text_metadata):
            results.append({
                ...
                "entity_ids": text_metadata[idx].get("entity_ids", []),
                "relationship_ids": text_metadata[idx].get("relationship_ids", [])
            })
    return results
...
async def initialize_faiss_index(rebuild: bool = False):
    ...
    text_units = load_text_units_from_parquet()
...
async def lifespan(app: FastAPI):
    try:
        logger.info("正在初始化TextRAG服务...")
        await initialize_faiss_index(rebuild=False)
        logger.info("TextRAG服务初始化完成")
    except Exception as e:
        logger.error(f"初始化失败: {e}")
        raise
    yield
    logger.info("正在关闭TextRAG服务...")


app = FastAPI(lifespan=lifespan, title="TextRAG FAISS Service")
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    formatted_response = "# TextRAG检索结果:\n\n"
    for i, result in enumerate(results, 1):
        formatted_response += f"## 相关片段 {i} (相似度: {result['score']:.4f}):\n"
...
async def list_models():
    return JSONResponse(content={"object": "list", "data": [{"id": "faiss-text-search:latest", "object": "model", "created": int(time.time()), "owned_by": "faiss-textrag"}]})
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"在端口 {PORT} 上启动TextRAG FAISS服务")
